...
STOP_LIST_FILENAME = "~/.phenny/list/stop.txt"
...
STUMPED_LIST = ["hmm?", "huh?", "excuse me?", "sorry, what?"]
...
def search(parser, input):
    ...
    for result in result_list:
        ...
        if len(message.split()) < 4:
            continue
        for word in BAD_WORD_LIST:
            message = message.replace(word, "foonickbar")
        message_list.append(message)
        word_list = word_list + message.split()
    return message_list, word_list

def filter(input):
    input = re.compile(r"(?i)\bfoonickbar\b").sub("", input)
...
def response_add_self(response):
    if random.randint(1, 2) == 1:
        response = re.compile(
            r"(\ba\b )(\w+)(.*)").sub("\g<1>foonickbar\g<3>", response)
    return response
...
def speak(phenny, input):
    if input.sender.startswith("#"):
        if not "foonickbar" in input.lower(): return
    for thanks in THANKS_LIST:
        if input == "%s foonickbar" % thanks:
